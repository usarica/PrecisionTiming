...
void checkGenMatching(){
  ...
  addFilesInDirectory(treeList.back(), "/hadoop/cms/store/user/usarica/MTD/UPS/MuonIsolation/20181024/DYToLL-M-50_0J_14TeV-madgraphMLM-pythia8/crab_DY_MuonIsolationMTD_200PU_932_HGCparam_new_v5");
  strSampleTitle.emplace_back("DY");
  strSampleLabel.emplace_back("DY");

  treeList.push_back(new TChain("muon_tree_30"));
  addFilesInDirectory(treeList.back(), "/hadoop/cms/store/user/usarica/MTD/UPS/MuonIsolation/20181024/TT_TuneCUETP8M2T4_14TeV-powheg-pythia8/crab_TTbar_ext_MuonIsolationMTD_200PU_932_HGCparam_new_nonlocal_v5");
  strSampleTitle.emplace_back("ttbar");
  strSampleLabel.emplace_back("t#bar{t}");

  TFile* foutput = TFile::Open("MuonGenMatchingTest.root", "recreate");

  unsigned int const nSamples = treeList.size();
  for (unsigned int is=0; is<nSamples; is++){
    float recoMuon_genMuon_DeltaR;
    float recoMuon_pt;
    float recoMuon_eta;
    float recoMuon_phi;
    float recoMuon_t;
    float recoMuon_terr;
    float recoMuon_AbsIso3D_ACOthers;
    float recoMuon_AbsIso3D_dtmuoncut_ACOthers;
    float recoMuon_AbsIso3D_hasdtmuon_dtmuoncut_ACOthers;
    bool recoMuon_isLooseMuon;
    bool recoMuon_isMediumMuon;
    bool recoMuon_isTightMuon3D;
    bool recoMuon_isTightMuon4D;
    float genMuon_pt;
    float genMuon_eta;
    float genMuon_phi;
    bool genMuon_isPromptFinalState;
    bool genMuon_isDirectPromptTauDecayProductFinalState;

    TTree tout(strSampleTitle.at(is), "");
    tout.Branch("recoMuon_genMuon_DeltaR", &recoMuon_genMuon_DeltaR);
    tout.Branch("recoMuon_pt", &recoMuon_pt);
    tout.Branch("recoMuon_eta", &recoMuon_eta);
    tout.Branch("recoMuon_phi", &recoMuon_phi);
    tout.Branch("recoMuon_t", &recoMuon_t);
    tout.Branch("recoMuon_terr", &recoMuon_terr);
    tout.Branch("recoMuon_AbsIso3D_ACOthers", &recoMuon_AbsIso3D_ACOthers);
    tout.Branch("recoMuon_AbsIso3D_dtmuoncut_ACOthers", &recoMuon_AbsIso3D_dtmuoncut_ACOthers);
    tout.Branch("recoMuon_AbsIso3D_hasdtmuon_dtmuoncut_ACOthers", &recoMuon_AbsIso3D_hasdtmuon_dtmuoncut_ACOthers);
    tout.Branch("recoMuon_isLooseMuon", &recoMuon_isLooseMuon);
    tout.Branch("recoMuon_isMediumMuon", &recoMuon_isMediumMuon);
    tout.Branch("recoMuon_isTightMuon3D", &recoMuon_isTightMuon3D);
    tout.Branch("recoMuon_isTightMuon4D", &recoMuon_isTightMuon4D);
    tout.Branch("genMuon_pt", &genMuon_pt);
    tout.Branch("genMuon_eta", &genMuon_eta);
    tout.Branch("genMuon_phi", &genMuon_phi);
    tout.Branch("genMuon_isPromptFinalState", &genMuon_isPromptFinalState);
    tout.Branch("genMuon_isDirectPromptTauDecayProductFinalState", &genMuon_isDirectPromptTauDecayProductFinalState);
    ...
    for (int ev=0; ev<nEntries; ev++){
      ...
      for (auto const& rgpair:reco_gen_pairs){
        const unsigned int& rmuon = rgpair.first.first;
        const unsigned int& gmuon = rgpair.first.second;
        TLorentzVector pReco; pReco.SetPtEtaPhiM(muon_pt->at(rmuon), muon_eta->at(rmuon), muon_phi->at(rmuon), 0);
        TLorentzVector pGen(genmuon_px->at(gmuon), genmuon_py->at(gmuon), genmuon_pz->at(gmuon), sqrt(pow(genmuon_px->at(gmuon), 2)+pow(genmuon_py->at(gmuon), 2)+pow(genmuon_pz->at(gmuon), 2)));

        recoMuon_genMuon_DeltaR = rgpair.second;
        recoMuon_pt=pReco.Pt();
        recoMuon_eta=pReco.Eta();
        recoMuon_phi=pReco.Phi();
        recoMuon_t=muon_t->at(rmuon);
        recoMuon_terr=muon_terr->at(rmuon);
        recoMuon_isLooseMuon=isLooseMuon->at(rmuon);
        recoMuon_isMediumMuon=isMediumMuon->at(rmuon);
        recoMuon_isTightMuon3D=isTightMuon3D->at(rmuon);
        recoMuon_isTightMuon4D=isTightMuon3D->at(rmuon);
        recoMuon_AbsIso3D_ACOthers=(muon_isosumtrackpt_vtx3D_associationrank_0->at(rmuon)+muon_isosumtrackpt_vtx3D_associationrank_1->at(rmuon)+muon_isosumtrackpt_vtx3D_unassociated->at(rmuon)); // Has dz cut
        recoMuon_AbsIso3D_dtmuoncut_ACOthers=(muon_isosumtrackpt_vtx3D_dtmuoncut_associationrank_0->at(rmuon)+muon_isosumtrackpt_vtx3D_dtmuoncut_associationrank_1->at(rmuon)+muon_isosumtrackpt_vtx3D_dtmuoncut_unassociated->at(rmuon)); // Has dz cut
        recoMuon_AbsIso3D_hasdtmuon_dtmuoncut_ACOthers=(muon_isosumtrackpt_vtx3D_hasdtmuon_dtmuoncut_associationrank_0->at(rmuon)+muon_isosumtrackpt_vtx3D_hasdtmuon_dtmuoncut_associationrank_1->at(rmuon)+muon_isosumtrackpt_vtx3D_hasdtmuon_dtmuoncut_unassociated->at(rmuon)); // Has dz cut
        genMuon_pt=pGen.Pt();
        genMuon_eta=pGen.Eta();
        genMuon_phi=pGen.Phi();
        genMuon_isPromptFinalState=genmuon_isPromptFinalState->at(gmuon);
        genMuon_isDirectPromptTauDecayProductFinalState=genmuon_isDirectPromptTauDecayProductFinalState->at(gmuon);
