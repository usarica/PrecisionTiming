...
void plotROCs(){
  ...
  vector<TString> strVarLabel{
    "Charged iso.(#Deltaz) wrt. 3D vtx.",
    "Charged iso.(#Deltaz,#Deltat) wrt. 4D vtx.",
    "SIP_{3D} wrt. 3D vtx.",
    "SIP_{3D} wrt. 4D vtx.",
    "#Deltat / #delta#Deltat"
  };
  ...
  std::vector<TH1F> sig_iso_dists; sig_iso_dists.reserve(nVars);
  std::vector<TH1F> bkg_iso_dists; bkg_iso_dists.reserve(nVars);
  std::vector<TGraph*> rocs;
  for (unsigned int iso=0; iso<nVars; iso++){
    if (iso<2){
      sig_iso_dists.emplace_back(Form("h_sig_isocut%i", iso), strSampleLabel.at(0), 500, 0, 2);
      bkg_iso_dists.emplace_back(Form("h_bkg_isocut%i", iso), strSampleLabel.at(1), 500, 0, 2);
    }
    else if (iso<4){
      sig_iso_dists.emplace_back(Form("h_sig_isocut%i", iso), strSampleLabel.at(0), 500, 0, 15);
      bkg_iso_dists.emplace_back(Form("h_bkg_isocut%i", iso), strSampleLabel.at(1), 500, 0, 15);
    }
    else{
      sig_iso_dists.emplace_back(Form("h_sig_isocut%i", iso), strSampleLabel.at(0), 500, 0, 15);
      bkg_iso_dists.emplace_back(Form("h_bkg_isocut%i", iso), strSampleLabel.at(1), 500, 0, 15);
  ...
  std::vector<float>* chIsoZTCut_3sigma=0;
  std::vector<float>* chIsoZCut=0;
  float vz, vtx4DZ, vtx3DZ, eta, pt, IP3DVtx4D, IP3DVtx3D, dIP3DVtx4D, dIP3DVtx3D, vtx4DT, vtx4DTerr, t0, t0err;
  bool genMatchedPrompt, genMatchedJet, isLooseMuon;
  tree_sig->SetBranchAddress("chIsoZTCut_3sigma", &chIsoZTCut_3sigma);
  tree_sig->SetBranchAddress("chIsoZCut", &chIsoZCut);
  ...
  tree_sig->SetBranchAddress("IP3DVtx4D", &IP3DVtx4D);
  tree_sig->SetBranchAddress("IP3DVtx3D", &IP3DVtx3D);
  tree_sig->SetBranchAddress("dIP3DVtx4D", &dIP3DVtx4D);
  tree_sig->SetBranchAddress("dIP3DVtx3D", &dIP3DVtx3D);
  ...
  tree_bkg->SetBranchAddress("chIsoZTCut_3sigma", &chIsoZTCut_3sigma);
  tree_bkg->SetBranchAddress("chIsoZCut", &chIsoZCut);
  ...
  tree_bkg->SetBranchAddress("IP3DVtx4D", &IP3DVtx4D);
  tree_bkg->SetBranchAddress("IP3DVtx3D", &IP3DVtx3D);
  tree_bkg->SetBranchAddress("dIP3DVtx4D", &dIP3DVtx4D);
  tree_bkg->SetBranchAddress("dIP3DVtx3D", &dIP3DVtx3D);
  ...
  TFile* foutput = TFile::Open("roc_test.root", "recreate");
  ...
  for (unsigned i = 0; i < nVars; ++i){
    ...
    for (int ev=0; ev<tree_sig->GetEntries(); ev++){
      ...
      switch (i){
      case 0:
        var = chIsoZCut->at(0)/pt;
        break;
      case 1:
        var = chIsoZTCut_3sigma->at(0)/pt;
        break;
      case 2:
        var = fabs(IP3DVtx3D)/dIP3DVtx3D;
        break;
      case 3:
        var = fabs(IP3DVtx4D)/dIP3DVtx4D;
        break;
      case 4:
        doSkip = !(t0err>0. && vtx4DTerr>0.);
        var = fabs(vtx4DT-t0)/sqrt(pow(t0err, 2)+pow(vtx4DTerr, 2));
        break;
      }
      float vtxZ = (use3D ? vtx3DZ : vtx4DZ);
      if (!doSkip && fabs(vz-vtxZ)<0.5 && genMatchedPrompt==1 && fabs(eta) < 2.4 && fabs(eta) > 1.5 && pt > 20 && isLooseMuon) sig_iso_dists[i].Fill(var);
    }
    for (int ev=0; ev<tree_bkg->GetEntries(); ev++){
      ...
      float vtxZ = (use3D ? vtx3DZ : vtx4DZ);
      if (!doSkip && fabs(vz-vtxZ)<0.5 && genMatchedJet==1 && genMatchedPrompt==0 && fabs(eta) < 2.4 && fabs(eta) > 1.5 && pt > 20 && isLooseMuon) bkg_iso_dists[i].Fill(var);
    }

    cout << sig_iso_dists[i].Integral() << endl;
    cout << bkg_iso_dists[i].Integral() << endl;
    rocs.push_back(createROCFromDistributions(&(sig_iso_dists[i]), &(bkg_iso_dists[i]), Form("ROC_%i", i)));
    rocs.back()->SetLineColor(colors[i]);
    rocs.back()->SetLineWidth(2);
    rocs.back()->SetMarkerColor(colors[i]);
    rocs.back()->SetMarkerStyle(20);
    rocs.back()->SetMarkerSize(0.8);
    if (!use3D) rocs.back()->SetLineStyle(7);

    foutput->WriteTObject(&(sig_iso_dists[i]));
    foutput->WriteTObject(&(bkg_iso_dists[i]));
    foutput->WriteTObject(rocs.back());
  }
