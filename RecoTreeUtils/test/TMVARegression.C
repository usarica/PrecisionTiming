...
void TMVARegression(TString myMethodList = "") 
{
   ...
   for(int iRec=0; iRec<=nRecHits; ++iRec)                                 
   {
       ...
       factory->AddVariable(timeBranch.Data(), timeBranch.Data(), "ns", 'F');
       if(iRec != 0)
       {
           //energyBranch = TString::Format(
           //  "energy_xstal_%.2d/TMath::Sqrt(TMath::Power(posx_xstal_%.2d-posx_xstal_00,2)+TMath::Power(posy_xstal_%.2d-posy_xstal_00,2))", iRec, iRec, iRec);
           // factory->AddVariable(posxBranch.Data(), posxBranch.Data(), "units", 'I');
           // factory->AddVariable(posyBranch.Data(), posyBranch.Data(), "units", 'I');
   ...
   if (Use["BDTG"])
     factory->BookMethod(TMVA::Types::kBDT, "BDTG",
                           "!H:!V:NTrees=2000::BoostType=Grad:Shrinkage=0.1:UseBaggedGrad:GradBaggingFraction=0.8:nCuts=20:MaxDepth=3:MaxDepth=4");
   // --------------------------------------------------------------------------------------------------
